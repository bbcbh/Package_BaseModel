...
/**
 * <p>
 * An infection objects for gonorrhoea, based on the specification listed in
 * Johnson et al. 2002
 * </p>
 *
 * <p>
 * The pathology of gonorrhoea is modelled using 5 stages. An <u>susceptible</u>
 * could be infected and become <u>exposed</u> to infection. An exposed can
 * becomes infectious with <u>symptom</u> or <u>asymptomatically</u>. Without
 * treatment, symptom will then be subdued, and become asymptomatic (or immune
 * immediately). All infections will recover naturally,and there will be a
 * period of <u>immunity</u> before the individual susceptible again.
 * </p>
 *
 * <p>
 * Reference:
 ...
 *
 * <pre>
 * Johnson, L.F., Alkema, L., Dorrington, R.E.: "A Bayesian approach to uncertainty
...
public class GonorrhoeaInfection extends infection.AbstractInfection implements infection.TreatableInfectionInterface {
	...
	public static final String[] PARAM_DIST_NAME = { "infectDur", "immuneDur", "symMale", "symFemale", "tranMF",
			"tranFM", "exposeDur", "symDur" };
	private final double[][] DEF_DIST_VAR = { DEFAULT_INFLECT_DURATION, DEFAULT_IMMUE, DEFAULT_SYM_M, DEFAULT_SYM_F,
	...
	public boolean setParameter(String id, Object value) {
		...
		if (m.find()) {
			...
			System.err.println(this.getClass().getName()
					+ ".setParameter : Value not a valid object for parameter id \"" + id + "\"");
			return false;
		}
	}
	...
	public double advancesState(AbstractIndividualInterface p) {
		...
		case STATUS_SYM:
			...
			// Y -> A or I
			/*
			 * double totalInfectDur = getRandomDistValue(DIST_INFECT_DUR_INDEX); if
			 * (totalInfectDur > infectedSince) { // Additional time as asymptomatic res =
			 * totalInfectDur - infectedSince; setInfection(p, STATUS_ASY, res); } else { //
			 * Become immune immediately // Unlikely if
			 * getRandomDistValue(DIST_INFECT_DUR_INDEX) >>
			 * getRandomDistValue(DIST_SYM_DUR_INDEX); res =
	...
	public boolean couldTransmissInfection(AbstractIndividualInterface src, AbstractIndividualInterface tar) {
		boolean transmission = isInfectious(src)
				&& (tar.getInfectionStatus(this.getInfectionIndex()) == AbstractIndividualInterface.INFECT_S);
		if (transmission) {
			double trans;
			if (tar.isMale()) {
